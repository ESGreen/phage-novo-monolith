...
class ProfileForm(forms.Form):
    ...
    photo = forms.ImageField(label="Replace photo", required=False)
    new_email = forms.EmailField(label="New email", required=False)
    confirm_new_email = forms.EmailField(label="Confirm new email", required=False)
    ...
    def clean(self) -> dict[str, object]:
        ...
        if not new_email and not confirm_new_email:
            return cleaned_data
        ...
        if not new_email:
            self.add_error("new_email", "Enter the new email address.")
        if not confirm_new_email:
            self.add_error("confirm_new_email", "Confirm the new email address.")
    ...
    def save(self) -> User:
        ...
        self.user.save(update_fields=["email", "first_name", "last_name", "updated_at"])
        ...
        self.profile.save(update_fields=["photo", "bio_markdown", "updated_at"])
